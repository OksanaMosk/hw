...
export const ProductComponent: FC<ProductComponentProps> = ({ product }) => {
  ...
  return (
      <li className="relative flex flex-col justify-between w-[600px] gap-0 text-[#404214] box-border bg-[#eef0d3] hover:text-[#ffffff] hover:bg-[#bab977] p-4 shadow-xl rounded-xl cursor-pointer hover:shadow-2xl hover:border-[1px]">
        ...
        <div className="flex justify-between w-full">
          ...
          <div className="">
            <div ref={mainRatingRef} className="flex gap-1"></div>
          </div>
        </div>
        ...
        <div className="flex justify-between w-full">
          <img className="imgHero" src={thumbnail.toString()} alt={title} />
        ...
        <div className=" flex gap-20 w-full my-2 border-t-white  border-t-[3px]  ">
          <div className="relative w-1/2 flex justify-center items-center mty-2 ">
              <button
                  onClick={handlePrev}
        ...
        <ul className="flex flex-col w-1/2 gap-1 text-[10px]">
          {reviews.map(({ rating, comment, date, reviewerName, reviewerEmail }: IReviews, index: number) => {
            const starRef = useRef<HTMLDivElement>(null);

            useEffect(() => {
              if (starRef.current) {
                renderStars(rating, starRef.current);
              }
            }, [rating]);

            return (
                <li
                    className="relative flex flex-col w-full gap-1 text-[10px] g-1 text-[#404214] box-border shadow-xl]"
                    key={index}
                >
